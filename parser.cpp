...
int main() {
  ...
  while (s.size() > 0) {
    ...
    const unsigned int length = stoul(s.substr(0, 2), nullptr, 16); // Todo: we assume that the length is at max 1 byte!!!
    ...
    if (type == "0a") {
      ...
      const u_int64_t func_size = stoul(s.substr(2, 2), nullptr, 16);
      const u_int64_t local_var_count = stoul(s.substr(4, 2), nullptr, 16);
